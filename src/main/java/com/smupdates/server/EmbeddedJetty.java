package com.smupdates.server;


import com.smupdates.AppStart;
import com.smupdates.config.PropertyLoaderListner;
...
public class EmbeddedJetty {
    ...
    private static final String CONFIG_LOCATION_PACKAGE = "com.smupdates.config";
    ...
    private void startJetty(int port) throws Exception {
        ...
        es.execute(new Thread() {
            public void run() {
                ...
                LOGGER.info("completed excecutor");
            }
        });
        server.join();
    ...
    getServletContextHandler() throws IOException {
        ...
        contextHandler.addEventListener(new PropertyLoaderListner());
    ...
    private static void addRuntimeShutdownHook(final Server server) {
        Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
            ...
            public void run() {
                if (server.isStarted()) {
                    try {
                        LOGGER.info("shutdown all process");
                        es.shutdown();
